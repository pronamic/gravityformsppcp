...
/* harmony import */ var _utilities_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./utilities.js */ "./js/src/utilities.js");
...
/* global gform, paypal, gform_ppcp_form_editor_strings, GFPayPalCheckoutUtils, HideSettings, SetFieldProperty, GetFieldsByType, GetSelectedField */
...
(function ($, strings) {
	window.GFPPCPFormEditor = function () {
		var self = this;
		var subscriptionsData = strings.form_data[form.id];
		...
		self.subscriptions = {
			...
			unsupportedPaymentMethod: function unsupportedPaymentMethod(value) {
		...
		self.renderSmartPaymentButtons = function (field) {
			var buttonsContainerID = '#gform_ppcp_smart_payment_buttons';
			var $buttonsContainer = $(buttonsContainerID);
			...
			self.paypalButtons.render(buttonsContainerID).then(function () {
				self.isRenderingButtons = false;
			});
		};
   ...
   */
		...
		self.bindRerenderSmartPaymentButtons = function (field) {
			$('#smart_payment_buttons_container select').off('change').on('change', function () {
		...
		self.bindLoadFieldSettings = function () {
			$(document).bind('gform_load_field_settings', function (event, field, form) {
				...
				for (var _i in field.creditCards) {
					if (!field.creditCards.hasOwnProperty(_i)) {
						continue;
					}
				...
				var activeImage = 'active1.png';
				var inactiveImage = 'active0.png';

				var imagesUrl = strings.imgurl,
				...
				$('.sub_labels_setting.paypal .input_active_icon.cardholder_name').off('click keypress').on('click keypress', function (e) {
					e.stopImmediatePropagation();
					var $img = $(this);

					var isActive = $img.attr('src').indexOf(activeImage) >= 0;
					var currentImage = isActive ? activeImage : inactiveImage;
					var replacmentImage = isActive ? inactiveImage : activeImage;
					var newSrc = $img.attr('src').replace(currentImage, replacmentImage);
   ...
   */
		self.SetPaymentMethods = function (e, elem) {
			...
			var value = $(elem).val();
			var methodOptionsContainerId = $(elem).attr('data-method-options');
			if ($(elem).is(':checked')) {
   ...
   */
		self.maybeDisplayUnsupportedPaymentMethodMessage = function (elem, method) {
			var $unsupportedMethod = $(elem);
			var $errorMessage = $(this.fieldContainer).find('#gf-paypal-checkout-unsupported-payment-warning');
   ...
   */
		...
		self.bindSetPaymentMethod = function () {
			$('.supported_payment_methods input:checkbox:not([data-option])').on('click keypress', function (e) {
