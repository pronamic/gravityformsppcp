...
/* harmony import */ var babel_runtime_core_js_object_values__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! babel-runtime/core-js/object/values */ "./node_modules/babel-runtime/core-js/object/values.js");
/* harmony import */ var babel_runtime_core_js_object_values__WEBPACK_IMPORTED_MODULE_5___default = /*#__PURE__*/__webpack_require__.n(babel_runtime_core_js_object_values__WEBPACK_IMPORTED_MODULE_5__);
/* harmony import */ var _state_manager__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./state_manager */ "./js/src/state_manager.js");
...
/* global paypal, gf_check_field_rule, gformIsRecaptchaPending, gformCalculateTotalPrice, gforms_ppcp_frontend_strings */
...
(function ($) {
	...
	window.GFPPCP = function (args) {
		...
		self.init = function () {
			...
			gform.ppcp.stateManager = new _state_manager__WEBPACK_IMPORTED_MODULE_6__["default"](initialState, self.handleStateChange.bind(this));
   ...
   *
   * @returns {array} Returns an associative array of payment methods.
   */
		self.getPaymentMethods = function (activeFeed) {
			...
			var paymentMethods = [];
			babel_runtime_core_js_object_values__WEBPACK_IMPORTED_MODULE_5___default()(activeFeed.supportedPaymentMethods).forEach(function (paymentMethod) {
				var method = paymentMethod.value ? paymentMethod.value : paymentMethod;
				if (babel_runtime_core_js_object_values__WEBPACK_IMPORTED_MODULE_5___default()(self.paymentMethods).includes(method)) {
   ...
   */
		self.showHideFields = function (state, paymentAmount) {
			...
			var paymentMethods = state['activeFeed'] ? self.getPaymentMethods(state['activeFeed']) : [];

			var isPaymentSelectorVisible = self.showHidePaymentMethodSelector(paymentMethods);
			var isCreditCardEnabled = self.isCreditCardEnabled(state['activeFeed']);
			var hasCreditCardPaymentMethod = isPaymentSelectorVisible || state['paymentMethod'] === 'Credit Card' && isCreditCardEnabled;
			var hidePayPalField = !state['fieldVisible'] || paymentAmount == 0 || !hasCreditCardPaymentMethod;

			// Display or show main PayPal field based on conditional logic or payment amount.
			var fieldDisplay = hidePayPalField ? 'none' : 'block';
			self.showHidePayPalField(fieldDisplay);

			var creditCardField = $('#gform_wrapper_' + self.formId + ' .ginput_container_custom_card_fields');
			if (state['paymentMethod'] === 'Credit Card') {

				var creditCardFieldDisplay = isCreditCardEnabled ? 'block' : 'none';
				creditCardField.css('display', creditCardFieldDisplay);
				...
				var hidePayPalButton = !state['fieldVisible'] || paymentAmount == 0 || !self.isPayPalButtonsEnabled(state['activeFeed']);

				// Display PayPal Button if conditional logic and amount > 0 are satisfied.
				var buttonDisplay = hidePayPalButton || !state['submitVisible'] ? 'none' : 'inline-block';
				self.paypalButtons.css('display', buttonDisplay);

				var submitVisibility = hidePayPalButton && state['submitVisible'] ? 'visible' : 'hidden';
   ...
   */
		self.showHidePayPalField = function (display) {
			var paypalField = $('#gform_wrapper_' + self.formId + ' .gform_ppcp_custom_card_fields');

			var prevDisplay = paypalField.css('display');
   ...
   */
		self.showHideSubmitButton = function (visibility) {
			var submitButton = $(self.submitButtonId);
   ...
   */
		self.renderPayPalFields = function (activeFeed, paymentAmount) {
			...
			var isCreditCardRendered = $(self.form).data('ppcp-card-rendered');
   ...
   */
		self.isCreditCardEnabled = function (activeFeed) {
			var paymentMethods = self.getPaymentMethods(activeFeed);

			return typeof paypal !== 'undefined' && paypal.HostedFields.isEligible() && paymentMethods.includes('Credit Card');
		};
   ...
   *
   * @param {array} activeFeed  Current active feed.
   * @returns {bool} True if PayPal buttons can be displayed. False otherwise.
   */
   ...
   *
   * @returns {float} Returns the filtered total.
   */
   ...
   */
		self.handleFrontendFeedsEvaluated = function (feeds, formId) {
			...
			var activeFeed = self.getActiveFeed(feeds);
			gform.ppcp.stateManager.set('activeFeed', activeFeed);

			// See if payment method needs to change as well.
			var paymentMethods = self.getPaymentMethods(activeFeed);
			var paymentMethodChanged = paymentMethods.length > 0 && !paymentMethods.includes(self.paymentMethodField.val());
   ...
   * @since 2.4
   *
   * @returns {bool} Returns true if PayPal fields have been rendered. Returns false otherwise.
   */
   ...
   */
		self.renderCreditMessage = function (paymentAmount) {
			...
			var messagePlaceholder = $('#gform_wrapper_' + self.formId + ' #paypal_credit_messages');
   ...
   */
		self.renderPayPalButtons = function (intent, paymentAmount) {
			...
			// Render buttons.
			var btnArgs = self.getPayPalButtonArgs(intent);
			self.updatePayPalScriptIntent(intent);

			var button = paypal.Buttons(btnArgs);
   ...
   * @since 2.4
   *
   * @returns {float} Returns the payment amount. Takes into account which product is configured in the feed.
   */
		self.getPaymentAmount = function (activeFeed, total) {
			var paymentAmount = total;

			if (activeFeed.paymentAmount !== 'form_total') {
				var price = GFMergeTag.getMergeTagValue(self.formId, activeFeed.paymentAmount, ':price'),
		...
		self.renderCardFields = function () {
			...
			paypal.HostedFields.render({
				...
				hf.on('cardTypeChange', function (event) {
					...
					var cardTypeInput = $('#ppcp_credit_card_type_' + self.formId);
   ...
   */
		self.handleFormSubmission = function (event) {
			...
			var continueDefaultFormSubmission = !gform.ppcp.stateManager.get('activeFeed') || gform.ppcp.stateManager.get('paymentMethod') !== 'Credit Card' || $('#gform_save_' + self.formId).val() === '1' || $(self.form).data('gfppcpsubmitting') || self.paymentAmount <= 0 || !self.isCreditCardOnPage() || self.isGoPrevPage() || typeof gformIsRecaptchaPending !== 'undefined' && gformIsRecaptchaPending($(self.form));
   ...
   */
		self.resetPayPalScript = function (currentCurrency, scriptLoadedCallback) {
			...
			var paypalScript = $('#gform_paypal_sdk-js');
			...
			var sdkNewCurrencySrc = paypalScript.attr('src').replace(/currency=\w{3}/, 'currency=' + currentCurrency);
			var attributionId = paypalScript.attr('data-partner-attribution-id');
			...
			$('<script>').attr('id', 'gform_paypal_sdk-js').appendTo('head');
   ...
   */
		self.getActiveFeed = function (feeds) {
			...
			var activePPCPFeeds = feeds.filter(function (feed) {
			...
			var activeFeeds = babel_runtime_core_js_object_values__WEBPACK_IMPORTED_MODULE_5___default()(self.feeds).filter(function (feed) {
   ...
   */
		self.getPayPalButtonArgs = function (intent) {
			...
			var btnArgs = {
				...
				onInit: function onInit(buttonProperties) {
					self.enableForm();
				},
   ...
   */
		self.applyPriceClass = function () {
			...
			var fieldsValue = babel_runtime_core_js_object_values__WEBPACK_IMPORTED_MODULE_5___default()(window['gf_form_conditional_logic'][self.formId].fields);
			var fields = [];
    ...
    */
		...
		self.updateOrderDetails = function () {
			var activeFeed = gform.ppcp.stateManager.get('activeFeed');
			...
			try {
				for (var _iterator = babel_runtime_core_js_get_iterator__WEBPACK_IMPORTED_MODULE_3___default()(self.items), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
   ...
   */
		self.disableForm = function () {
			...
			$(self.form).data('gform-disabled', true).find('input, select, textarea').each(function () {
   ...
   */
		self.enableForm = function () {
			...
			$(self.form).data('gform-disabled', false).find('input, select, textarea').each(function () {
   ...
   */
		self.createSubscription = function () {
			var _ref2 = babel_runtime_helpers_asyncToGenerator__WEBPACK_IMPORTED_MODULE_2___default()( /*#__PURE__*/babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default.a.mark(function _callee2(data, actions) {
					...
					while (1) {
						switch (_context2.prev = _context2.next) {
							case 0:
								...
								subscriptionRequest = '';
								_context2.next = 5;
								return fetch(gforms_ppcp_frontend_strings.ajaxurl + '?action=gfppcp_create_subscription', {
									method: 'POST',
									headers: {
										'content-type': 'application/json'
									},
							...
							case 5:
								...
								return _context2.abrupt('return', actions.subscription.create(subscriptionRequest));
   ...
   */
		...
		self.displayCardError = function (message) {
			...
			var messageContainer = self.getMessageContainerSelector();
			...
			var cardErrors = self.getCardErrors();
   ...
   */
		self.getCurrentCurrencyCode = function () {
			var code = '';
			var currentCurrency = new Currency(gf_global.gf_currency_config);

			if ('getCode' in currentCurrency === false || typeof currentCurrency.getCode !== 'function') {
				var supportedCurrencies = gforms_ppcp_frontend_strings.currencies;
				var _iteratorNormalCompletion2 = true;
				var _didIteratorError2 = false;
				var _iteratorError2 = undefined;

				try {
					for (var _iterator2 = babel_runtime_core_js_get_iterator__WEBPACK_IMPORTED_MODULE_3___default()(babel_runtime_core_js_object_keys__WEBPACK_IMPORTED_MODULE_4___default()(supportedCurrencies)), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
						var supportedCurrencyCode = _step2.value;
   ...
   */
		self.destroyPayPalButtons = function () {
			...
			try {
				...
				babel_runtime_core_js_object_keys__WEBPACK_IMPORTED_MODULE_4___default()(window).forEach(function (key) {
		...
		self.closePayPalButtons = function () {
			...
			try {
				for (var _iterator3 = babel_runtime_core_js_get_iterator__WEBPACK_IMPORTED_MODULE_3___default()(self.paypalButtonInstances), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
					var buttonInstance = _step3.value;
 ...
 */
var PPCPStateManager = function () {
	function PPCPStateManager(initialState, stateChangeCallback) {
		babel_runtime_helpers_classCallCheck__WEBPACK_IMPORTED_MODULE_1___default()(this, PPCPStateManager);

		this.state = {};
		this.prevState = babel_runtime_helpers_extends__WEBPACK_IMPORTED_MODULE_0___default()({}, initialState);
		this.markedDirty = [];
  ...
  */
	...
	babel_runtime_helpers_createClass__WEBPACK_IMPORTED_MODULE_2___default()(PPCPStateManager, [{
		key: "get",
   ...
   */
		...
		value: function _reset() {
			this.prevState = babel_runtime_helpers_extends__WEBPACK_IMPORTED_MODULE_0___default()({}, this.state);
			this.timeoutID = 0;
   ...
   */
		...
		key: "_invokeStateChanged",
		value: function _invokeStateChanged() {
			var dirtyProps = this._getDirtyProps();
   ...
   */
		...
		value: function _maybeClearTimeout() {
   ...
   *
   * @returns {string[]} Returns an array with keys that have been changed or that have been manually marked as dirty.
   *
   * @private
   */
	...
	return PPCPStateManager;
}();

/* harmony default export */ __webpack_exports__["default"] = (PPCPStateManager);
